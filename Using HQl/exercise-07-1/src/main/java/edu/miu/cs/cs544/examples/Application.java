...
import edu.miu.cs.cs544.models.Airline;
import edu.miu.cs.cs544.models.Airplane;
import edu.miu.cs.cs544.models.Airport;
import edu.miu.cs.cs544.models.Flight;
...
public class Application {
	...
	static {
		...
		sessionFactory = HibernateUtils.getSessionFactory(Arrays.asList(Flight.class,Airport.class,Airline.class,Airplane.class));
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			tx = session.beginTransaction();
	// ---------------07_1.a-----------------------------------------------------------------------------//		
			 List<Flight> flights = session.createQuery(" FROM Flight f where f.origin.country = 'USA' and f.airplane.capacity > 500 ").list();
			 System.out.println("Flight:  Departs:     "
	            ...
	            for (Flight flight : flights) {
	                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s %8s\n",
	                        ...
	                        .getArrivalDate(), flight.getArrivalTime(), flight.getAirplane().getCapacity());
	            }
			
  // ---------------07_1.b-----------------------------------------------------------------------------//		
	         List<Airplane> airplanes = session.createQuery(" From Airplane  where model = 'A380' ").list();
 ...
 // ---------------07_1.c--------------------------------------------------------------------------------//		
	         ...
	         List<Flight> flights_2 = session.createQuery("From Flight f where f.airplane.model = 'A380' and f.airline.name != 'Star Alliance' ").list();  
	         System.out.println("Flight:  Departs:     "
	                    + "  Arrives:         Model:  Airline:");
...
// ---------------07_1.d-----------------------------------------------------------------------------------//
				...
				List<Flight> flights_3 = session.createQuery("from Flight f where f.departureTime < '12:00:00' "
						+ "and f.departureDate = '2009-08-07'").list();
