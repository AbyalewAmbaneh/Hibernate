...
import cs544.exercise07_1.models.Airline;
import cs544.exercise07_1.models.Airplane;
import cs544.exercise07_1.models.Airport;
import cs544.exercise07_1.models.Flight;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<Flight> flights = session.createQuery("Select f from Flight f where f.origin.country = 'USA' "
            		+ "and f.airplane.capacity > 500").list();
        ...
        try {
            ...
            List<Airline> airlines = session.createQuery("Select distinct a "
            		+ "from Airline a "
            		+ "join a.flights f join  f.airplane r "
            		+ "where r.model = 'A380' ").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("Select f from Flight f where f.airplane.model = 747" 
            +"and f.airline.name != 'Star Alliance'" ).list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
...
//        // d) All flights leaving before 12pm on 08/07/2009
        try {
            ...
            // TODO update HQL
			@SuppressWarnings("unchecked")
			List<Flight> flights = session.createQuery("from Flight f where f.departureTime < '12:00:00' "
